...
public class StraightLineGeometryManager extends AbstractLineGeometryManager {
	public Shape getShapeForMessage(ID src, ID dest) {
		return super.getShapeForMessage(src, dest, 0.0);
	}

	public Shape getShapeForConnection(ID src, ID dest) {
		return super.getShapeForConnection(src, dest, 0.0);
	}

	public Point2D getNodePoint2D(ID id) {
		return super.getNodePoint2D(id.toBigInteger().doubleValue(), 0);
	}
}
