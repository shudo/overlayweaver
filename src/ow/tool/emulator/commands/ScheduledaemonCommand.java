...
public final class ScheduledaemonCommand extends AbstractScheduleCommand {
	private final static String[] NAMES = {"scheduledaemon", "scheddaemon"};
	...
	public String getHelp() {
		return "scheduledaemon [+]<time>[,<interval>[,<times>]] <another command> [...]";
	}

	void schedule(EmulatorContext cxt,
			long relativeTime, long interval, int times, EmulatorTask task,
			boolean timeIsDifferential) {
		cxt.scheduleTask(relativeTime, interval, times, task, timeIsDifferential, true);
	}
}
