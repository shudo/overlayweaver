...
public class CHTProvider implements DHTProvider {
	private final static String[] DHT_NAMES = { "CHT" };
	...
	public <V extends Serializable> DHT<V> getDHT(
			short applicationID, short applicationVersion, DHTConfiguration config, ID selfID)
				throws Exception {
		return new CHTImpl<V>(applicationID, applicationVersion, config, selfID);
	}

	public <V extends Serializable> DHT<V> getDHT(DHTConfiguration config, RoutingService routingSvc)
			throws Exception {
		return new CHTImpl<V>(config, routingSvc);
	}
}
